...
class DownloadStream:
    __slots__ = ("chunks", "_content_type", "_content_length", "headers")

    def __init__(self, chunks, content_type=None, content_length=None, *, headers=None):
        self.chunks = chunks
    ...
    def content_type(self):
        return self._content_type or self._header("content-type")
...
async def image_from_result(
    ...
    *,
    ...
    def validate_metadata():
        ...
        if response.content_length is not None:
            try:
                ...
                raise ImageOutputError("download content length is invalid") from exc
    ...
    async for chunk in response.chunks:
        interrupt()
        # transport.http populates its shared header mapping immediately before
        # a zero-byte metadata barrier.  Re-check here so streamed MIME/length
        # metadata is enforced before any provider body bytes are accepted.
        validate_metadata()
        ...
        with Image.open(io.BytesIO(raw)) as image:
            ...
            if has_alpha:
                ...
                if alpha_min != 255 or alpha_max != 255:
                    raise ImageOutputError(
                        "provider output contains transparency, but this node declares only IMAGE; alpha was not discarded"
                    )
                image = rgba
