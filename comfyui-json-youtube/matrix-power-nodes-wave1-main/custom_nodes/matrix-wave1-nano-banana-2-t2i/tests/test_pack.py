...
class PackWiringTests(unittest.TestCase):
    def test_declared_nodes_are_registered(self):
        ...
        for node_id, module in {'MATRIX_Wave1NanoBanana2T2i': 'matrix_wave1nanobanana2t2i'}.items():
    ...
    def test_route_widget_order_is_published(self):
        expected = {'MATRIX_Wave1NanoBanana2T2i': ['prompt', 'aspect_ratio', 'enable_image_search', 'enable_web_search', 'output_format', 'resolution']}
        for node_id, route_names in expected.items():
            module = {'MATRIX_Wave1NanoBanana2T2i': 'matrix_wave1nanobanana2t2i'}[node_id]
