...
class PackWiringTests(unittest.TestCase):
    def test_declared_nodes_are_registered(self):
        ...
        for node_id, module in {'MATRIX_Wave1NanoBananaProEdit': 'matrix_wave1nanobananaproedit'}.items():
    ...
    def test_route_widget_order_is_published(self):
        expected = {'MATRIX_Wave1NanoBananaProEdit': ['prompt', 'images', 'aspect_ratio', 'output_format', 'resolution']}
        for node_id, route_names in expected.items():
            module = {'MATRIX_Wave1NanoBananaProEdit': 'matrix_wave1nanobananaproedit'}[node_id]
