...
- **Что доступно локально**: `git`, `gh`, `perl`, стандартные утилиты GNU. На NixOS дополнительные инструменты запускаются разово через `nix-shell -p <pkg> --run '...'`; систему и пользовательскую конфигурацию этим не изменяем.
- **Инструменты проекта**: наличие `python`, `node`, `npm`, `tsc` и линтеров в PATH не предполагается. Если инструмент нужен для дешёвой проверки, запускаем его через существующую Nix-среду или `nix-shell`.
- **🚫 Бюджет локальных проверок**:
  ...
  * **Дешёвые проверки разрешены и приветствуются**: `python -m compileall <изменённые файлы>`, линтер, `tsc --noEmit`, `npm run build`, отдельный тест-кейс без БД. Если бинарников нет в PATH, используем `nix-shell`; их цена — секунды CPU, и они ловят синтаксис и опечатки до того, как те попадут в репозиторий.
...
- Вёрстка должна держать 390/768/1024/1440px без document-level overflow. Локальный браузер разрешён для визуальной проверки, если он запускается в отдельном профиле и не читает или изменяет пользовательский профиль браузера. Захваченные страницы сторонних продуктов и локальные HTML-снимки в репозитории не хранятся.
