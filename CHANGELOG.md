...
- Statistics: строка с объёмом результатов ниже порога решения (10) помечается как «данных недостаточно» — это отдельное утверждение от «работает плохо», и такая строка не окрашивается как проблема. Целевая цена результата пока не хранится в API, поэтому строки показывают цену без вердикта, а отсутствие цели сообщается один раз в обзоре, а не в каждой строке.
