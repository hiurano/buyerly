...
### Added & Improved
- **Linear-Style 2-Tier Sidebar & Motion Physics**:
  - Реализована двухслойная DOM-архитектура левого сайдбара (внешний flex-слот + независимый абсолютный холст `[data-sidebar-surface]`).
  - Перенесены точные токены анимации Linear: `--speed-slowTransition: 0.32s` и `--ease-out-quint: cubic-bezier(0.16, 1, 0.3, 1)` для шелковистого GPU-скольжения без резких рывков.
  - Сохранена 8px сетка Linear: при сворачивании внешний слот сжимается до 8px, обеспечивая идеальный зазор контентного холста.
  - Добавлена бесшовная интерполяция кнопки Menu (`width: 0 -> 28px`, `opacity: 0 -> 1`, `scale: 0.85 -> 1`) в шапках представлений при фиксированном padding без скачков заголовков.
  - Синхронизирован активный drag ресайзера: отключены CSS transitions на время перетаскивания (`data-resizing="true"`), граница и 1px трек зафиксированы пиксель-в-пиксель за курсором.
  - Подключен глобальный хоткей `[` (и `х`/`Х` на русской раскладке) для мгновенного переключения сайдбара из любого места приложения.
  - Шапка открытого сайдбара приведена к эталону Linear (только Workspace, Search `/` и Create `C`).
  - Улучшен 3-слойный Spotlight-ресайзер: добавлен Snap-to-Collapse при сдвиге влево `< 150px` и сброс ширины к базовым `244px` по двойному клику (`dblclick`).
- **Linear-style filters for Ads Manager and Rules**:
  - Добавлена единая типизированная модель условий и общий predicate engine для Campaigns, Ad sets, Ads и Rules с AND между свойствами и OR внутри мультизначения.
  - Реализованы каскадные меню свойств/значений, остающийся открытым мультивыбор с checkbox, быстрый поиск, редактируемые сегменты `свойство | оператор | значение`, добавление через `+`, удаление отдельных условий и полный сброс.
  - Ads Manager использует сфокусированный каталог `Status / Groups / Rules` без нерелевантного Platform и полей ручного ввода; Rules использует `Status / Groups`.
  - Shortcut `F` и быстрые Group/Rule-фильтры правой панели синхронизированы с общей формулой; иконки значений совпадают с соответствующими sidebar-элементами.
  - Rules list и board используют один отфильтрованный набор; добавлены согласованные hidden/empty states и доступные aria-labels.
