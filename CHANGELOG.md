...
### Changed
- CI разбит на параллельные части. Тестовый job стал матрицей из 4 шардов, каждый со своими Postgres и Redis, а модули распределяет `scripts/ci_test_shard.py`: жадная упаковка по весу, где async-тест считается в 10 раз дороже синхронного. Набор теперь стоит времени самого медленного шарда, а не суммы. Сборка фронта вынесена в отдельный job `frontend` — она дублировалась в каждом шарде, хотя ни один тест не читает `frontend/dist`.
