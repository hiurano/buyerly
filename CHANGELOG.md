...
### Added
- Белый список принимает username существующего аккаунта (#181): `POST /api/auth/admin/allowed-emails` с `{"username": ...}` вместо `email`. Ник ищется без учёта регистра и пробелов, разрешение хранится на `user_id` (новая колонка `allowed_emails.user_id`, миграция `0027_allowlist_user_grants`), поэтому смена ника не передаёт доступ новому владельцу. Неизвестный ник — 404, неоднозначный — 409, повтор не создаёт дубликат. Такой пользователь может создать первый workspace и через onboarding, и через `POST /api/workspaces`, а если у аккаунта есть подтверждённый email — войти и по коду из письма. Проверки whitelist вынесены в `services/allowlist.py`. Отзыв любой записи удаляет и остальные записи того же аккаунта, снимает одобрение и сессии, чтобы оставшееся разрешение не вернуло доступ; повторное добавление по нику возвращает одобрение. Записи по email работают как раньше.
...
### Fixed
- Правило больше не повторяет действие, которое байер отменил (#200). Раньше отмена в Inbox держалась до следующей проверки: правило без cooldown снова находило ту же кампанию или ad set и через окно подтверждения выключало его заново. Теперь до конца суток по часам рекламного кабинета правила не повторяют отменённое действие над этой сущностью: не выключают её снова, не включают снова, не меняют бюджет в ту же сторону. Worker берёт это из истории — успешного `UNDO_ACTION`, связанного с действием правила, — без новой таблицы и миграции; другие действия правил над сущностью работают. Перед отменой Inbox предупреждает об этом, а `POST /api/audit-events/{id}/undo` сообщает в `message`.
- Правило на cooldown больше не засоряет Inbox. Пока его условие держалось, оно каждые ~12 минут записывало «A STOP candidate was found» и «Action skipped: cooldown» — на суточном cooldown это около двухсот записей в сутки на одну кампанию. Теперь до конца cooldown правило не начинает подтверждение STOP и не пишет пропуски; ожидание и действие начинаются после cooldown.
- C05 ([PR #206](https://github.com/hiurano/buyerly/pull/206)): Statistics внутри кампании или ad set считает «сегодня», «вчера», сравнение и тренд по часам рекламного кабинета, как на уровне кабинета, а не по UTC. Раньше несколько часов в сутки (для кабинета в UTC−7 — каждый вечер с 17:00 до полуночи) drill-down показывал пустую таблицу или соседний день, а тренд подписывался «UTC». Кабинет кампании или ad set берётся из сохранённых фактов этого воркспейса и должен в нём состоять; неизвестный, чужой или оставшийся от удалённого кабинета родитель теперь получает `404` в `GET /api/analytics/hierarchy` и `GET /api/analytics/timeseries` вместо пустого ответа по UTC.
- Деплой больше не удаляет бэкап, снятый перед выкаткой ([PR #185](https://github.com/hiurano/buyerly/pull/185)). `scripts/deploy.sh` запускает обязательный `backup_db.sh`, а следом `git clean -ffd`; каталог `backups/` не был в `.gitignore`, поэтому с 27.08 (`d6c7ca3`) каждый деплой через несколько секунд стирал только что созданный архив из `/opt/buyerly/backups`. Теперь каталог игнорируется git и переживает деплой. `backup_db.sh` создаёт каталог и архивы с `umask 077`: без `BACKUP_ENCRYPTION_KEY` дамп хранится открытым текстом и не должен читаться другими пользователями сервера.
- C04 ([PR #184](https://github.com/hiurano/buyerly/pull/184)): строки `GET /api/analytics/hierarchy` возвращают собственный `parent_entity_id` из fact store вместо `parent_id` запроса. На account-wide запросе adsets снова привязаны к своим campaigns, а ads к своим adsets, поэтому таблицы Campaigns показывают правильные названия родителей. Добавлены backend-регрессия на две campaigns и check мапперов в CI.
