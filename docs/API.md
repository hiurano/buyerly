...
Ручные действия над сущностями (`/api/entities/...`) — единственный путь записи в Meta из веб-приложения. Роль viewer их не выполняет. Кабинет проверяется на принадлежность воркспейсу так же, как в остальных эндпоинтах: чужой отвечает `404` и пишет security-событие. Состояние до записи читается из Meta, а не берётся из запроса, и проверяется принадлежность сущности выбранному кабинету. Перед отправкой сохраняется PENDING-событие; без доступной истории запись блокируется. После ответа Meta событие становится SUCCESS или ERROR. При неизвестном результате нужно проверить состояние в Meta перед повтором. Записанное событие получает тип, который уже понимает контракт отмены (`MANUAL_PAUSE`, `MANUAL_REACTIVATE`, `INCREASE_BUDGET`, `DECREASE_BUDGET`), и отменяется через существующий `POST /api/audit-events/{event_id}/undo` в течение 24 часов.
...
| `POST /api/entities/{level}/{entity_id}/delivery` | `account_id`, `status=ACTIVE|PAUSED` | включает или выключает одну кампанию, ad set или объявление в Meta; текущее состояние читается из Meta перед записью, повторный запрос в то же состояние ничего не пишет и возвращает `changed=false`, а каждая попытка — успешная и неуспешная — попадает в историю; ответ содержит `audit_event_id` для отмены |
| `PATCH /api/entities/{level}/{entity_id}/budget` | `account_id`, `daily_budget` | меняет дневной бюджет той сущности, которая им владеет: кампании при CBO, ad set при ABO; бюджет только переносится, но не создаётся — сущность без бюджета отвечает `409`, объявление `400`, значение ниже единицы валюты кабинета `422` |
| `GET /api/analytics/timeseries` | `parent_id`, `level=campaign|adset|ad`, `days=2..30` | суточные итоги по всем сущностям под одним родителем, старый день первым; одна точка на каждую локальную дату окна, поэтому день, который хранилище не получало, помечен `has_data=false`, а не выглядит как день без расхода; `open_day` называет ещё не закрытый день, `currency` пуста при смешанных валютах; `parent_id` разбирается так же, как в `hierarchy`, поэтому даты, `open_day` и `timezone` берутся по часам рекламного кабинета родителя, а неизвестный или чужой родитель — `404` |
| `GET /api/analytics/hierarchy` | `parent_id`, `level=campaign|adset|ad`, `period=today|yesterday|last_3d|last_7d`, `compare=none|previous` | возвращает нормализованные дочерние сущности и метрики из Analytics Fact Store; campaign-уровень сохраняет полный Meta inventory, включая кампании без активности за период; `source` и `data_as_of` описывают источник и консервативную свежесть набора; `compare=previous` добавляет объект `comparison` и поле `previous` в каждую строку; `parent_entity_id` строки — её собственный родитель из fact store (account для campaign, campaign для adset, adset для ad), а не `parent_id` запроса; `parent_id` — рекламный кабинет воркспейса (уровень читается по всему кабинету) либо кампания или ad set, чей кабинет находится по сохранённым фактам этого воркспейса и тоже должен в нём состоять; окна периода и сравнения на любой глубине считаются по часам этого кабинета, иначе — `404` |
...
Отмена действия правила — решение байера против правила. До конца суток по часам рекламного кабинета правила не повторяют это действие над той же сущностью: не выключают её снова, не включают снова, не меняют бюджет в ту же сторону. Остальные действия правил над ней продолжают работать. Worker берёт это из истории, отдельного состояния нет. `message` ответа сообщает об этом, например: `Action undone and confirmed by Meta. Rules won't turn this ad set off again today.`
