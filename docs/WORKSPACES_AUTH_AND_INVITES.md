...
Публичной регистрации нет. Пользователь запрашивает вход на `/login`; доступ разрешается whitelist либо действующим приглашением. Письмо содержит одноразовую ссылку и код. Ссылка открывает `/auth/email/verify`; успешный обмен создаёт серверную сессию.
